...
def write_adapters_distributed_old(local_slice: PipelineSlice, model_config: dict) -> None:
    """
    Ring gather of LoRA adapters to rank 0 using neighbor-only sends/recvs.

    Algorithm (P ranks):
      - Build a *reference full model* (lazy) on every rank to derive a canonical
        list of expected LoRA keys & shapes for each rank's slice. (Cheap; lazy=True)
      - Perform P-1 rounds. In round with source=src and dest=src-1:
          * src sends the aggregated payload (params for ranks src..P-1) to dest
          * dest receives it and merges in its own local params
          * dest becomes the next src (src := dest)
      - On rank 0 at the end: ensure local params are present, apply to reference
        model, and write adapters + config.

    Only rank 0 writes to disk. Other ranks never touch the filesystem.
    """
    output_dir = model_config["output_location"]
    os.makedirs(output_dir, exist_ok=True)

    # -------- Build reference model + canonical metadata (on all ranks; lazy) --------
    # Build a *reference* full model (lazy) on every rank for canonical keys/meta.
    ...
    if "lora" in model_config:
        # Mirror training-time behavior: freeze base first, then inject LoRA
        lora_model(ref_model, model_config["lora"])
    inner = _inner_model(ref_model)
    prefix = _prefix_to_inner(ref_model)
    total_layers = _layers_total(inner)
    # Determine separators
    sep_ref = _detect_sep_from_module(ref_model)
    ...
    rel_keys_by_rank: Dict[int, List[str]] = {}
    global_meta: Dict[str, Tuple[Tuple[int, ...], mx.Dtype]] = {}
    ...
    for r in range(dist.size):
        ...
        # Only operate on LoRA tensors (robust to key separator differences)
        rel_items = [(k, v) for (k, v) in rel_items if "lora_" in k]
        # Record relative keys (in the same order as global keys below)
        rel_keys_by_rank[r] = [rk for rk, _ in rel_items]
        gks = []
        for rel_k, arr in rel_items:
            gk = _rekey_slice_to_global(rel_k, s, prefix, sep_ref)
            gks.append(gk)
            # record canonical shape/dtype (first writer wins; all should match)
            if gk not in global_meta:
                global_meta[gk] = (arr.shape, arr.dtype)
        global_keys_by_rank[r] = gks
    # Sanity: we must have discovered some LoRA tensors
    assert any(global_keys_by_rank.values()), (
        "No LoRA tensors discovered on reference model. "
        "This usually means path-separator detection failed."
    )

    # Helper: concatenated canonical order for "payload coming from src"
    # We send ranks [P-1, P-2, ..., src] in that order so that each round's payload
    # is a prefix of the previous round's payload (nice property for debugging).
    def payload_order_for_src(src: int) -> List[str]:
        order: List[str] = []
        for q in range(dist.size - 1, src - 1, -1):
            order.extend(global_keys_by_rank[q])
        return order

    # -------- Build my local map (global_key -> array) from my pipeline slice --------
    my_start, my_end = _rank_bounds(total_layers, dist.size, dist.rank)
    
    # Build a dict of *local* relative keys -> arrays (only LoRA)
    my_rel_dict_raw: Dict[str, mx.array] = {
    ...
    shadow_rel_list = [k for (k, _) in _sorted_rel_items(shadow_me) if "lora_" in k]
    rel2gk = { _canon_rel(rk): _rekey_slice_to_global(rk, my_start, prefix, sep_ref) for rk in shadow_rel_list}
    
    # Assemble local_map by name, with shape/dtype safety
    local_map: Dict[str, mx.array] = {}
    missing: List[str] = []
    for rk_canon, gk in rel2gk.items():
        arr = my_rel_dict.get(rk_canon)
        if arr is None:
            missing.append(rk_canon)
            continue
        shape, dtype = global_meta[gk]
        if arr.shape != shape or arr.dtype != dtype:
            arr = arr.astype(dtype).reshape(shape)
        local_map[gk] = arr
    
    assert not missing, (
        f"Rank {dist.rank}: missing {len(missing)} LoRA tensors expected in slice; "
        f"examples: {missing[:5]}"
    )

    # The in-flight aggregated payload on the current holder rank.
    # Maps canonical global key -> array.
    agg_map: Dict[str, mx.array] = {}

    # If size==1, we won't enter the loop—make sure we still write.
    # For P>1, rank 0 will also merge its local params after the final receive.
    if dist.size == 1:
        agg_map.update(local_map)

    # -------- Ring gather: P-1 rounds, neighbor-only traffic --------
    for src in range(dist.size - 1, 0, -1):   # src: P-1, P-2, ..., 1
        dest = src - 1
        order = payload_order_for_src(src)

        if dist.rank == src:
            # Ensure our own local tensors are in the aggregate before sending.
            for gk in global_keys_by_rank[src]:
                if gk not in agg_map:
                    agg_map[gk] = local_map[gk]

            send_tokens = []
            for gk in order:
                tok = mx.distributed.send(agg_map[gk], dst=dest)  # neighbor to the left
            ...
            # Receive the payload from my right neighbor (src)
            recv_bufs = []
            for gk in order:
                shape, dtype = global_meta[gk]
            ...
            # Merge my own local tensors into the aggregate so I become the next src
            for gk in global_keys_by_rank[dest]:
                if gk not in agg_map:
                    agg_map[gk] = local_map[gk]

        # Everyone sync before next hop
        dist.barrier()

    # -------- Rank 0 writes adapters --------
    if dist.rank == 0:
        # In case P==1, or as a safety net if rank 0 didn't merge earlier
        for gk in global_keys_by_rank[0]:
            if gk not in agg_map:
                agg_map[gk] = local_map[gk]

        # Apply per-slice using relative keys to avoid global-path mismatches
        for r in range(dist.size):
            s, e = _rank_bounds(total_layers, dist.size, r)
            gks = global_keys_by_rank.get(r, [])
            rks = rel_keys_by_rank.get(r, [])
            if not gks:
                continue
            shadow_r = PipelineSlice(ref_model, s, e)
            rel_payload: Dict[str, mx.array] = {}
            for rk, gk in zip(rks, gks):
                arr = agg_map[gk]
                shape, dtype = global_meta[gk]
                if arr.shape != shape or arr.dtype != dtype:
                    arr = arr.astype(dtype).reshape(shape)
                rel_payload[rk] = arr
            if rel_payload:
                shadow_r.update(rel_payload)
        mx.eval(ref_model.state)

        adapter_weights = dict(tree_flatten(ref_model.trainable_parameters()))
        # spot-check a few deeper-layer lora_b tensors:
        chk = [(k, v) for k, v in adapter_weights.items() if "lora_b" in k and ".12." in k][:2]
        for k, v in chk:
        ...
        adapter_config = {
            ...
            "lora_parameters": {
                "keys": list(get_lora_keys(ref_model)),
                **model_config["lora"],
            },
        }
    ...
    # Final sync (harmless when size==1)
    dist.barrier()
